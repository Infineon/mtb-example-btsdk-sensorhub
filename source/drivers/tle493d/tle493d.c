...
* File Name:   tle493d.c
*
* Description: This file shows implementation of 3d magnetic sensor interface.
*
 ...
 ******************************************************************************/
/* 3D magnetic sensor shield connection status*/

static volatile uint8_t mag3d_shield_connected = 0;
...
void mag3d_sensor_init(void)
{
    ...
    if(WICED_SUCCESS == response)
    {
        ...
        if(WICED_SUCCESS != response)
        {
            WICED_BT_TRACE("Failed to configure the 3D magnetic sensor\r\n");
        }
        mag3d_shield_connected = 1u;
    }
    else
    {
        WICED_BT_TRACE("Failed to read data or 3D magnetic sensor not connected\r\n");
    }

}
/*******************************************************************************
 * Function         mag3d_sensor_isConnected
 *
 * @brief           This function work on 3D magnetic sensor is connected  
 *
 * @return          : mag3d_shield_connected
 ******************************************************************************/
uint8_t mag3d_sensor_isConnected(void)
{
    return mag3d_shield_connected;
}
 ...
 * @return                       : None
 ******************************************************************************/
void mag3d_sensor_read_val (mag3axis_t *sense)
{
...
********************************************************************************
* Summary:
*  calculate the 3D magnetic flux value from raw data.
*
* Parameters:
*  uint16_t value : input data
*
*******************************************************************************/
float mag3dsense_calc_flux(uint16_t value){
